...
## 结构介绍
源自：https://sources.debian.org/src/glibc/2.28-10/malloc/malloc.c/  
这里可以在线看：https://code.woboq.org/userspace/glibc/malloc/malloc.c.html#malloc_chunk  
```c
/*
...
`|A|M|P|`3个bit的含义：  
A, NON_MAIN_ARENA, 不属于主线程内存则置1  
M, IS_MMAPPED, 通过mmap分配的内存则置1  
P, PREV_INUSE, 前一个chunk在使用则置1  

## 空间复用
当一个chunk处于使用状态时，它的下一个chunk的prev_size域肯定是无效的，
所以实际上，这个空间也可以被当前chunk使用，达到合理空间复用的目的。  
